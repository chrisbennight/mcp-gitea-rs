...
A `v*` tag push publishes its revision without advancing `latest`. An
...
recover a transient publication failure. Version-tagged releases and other
architectures are future work, not current artifacts.
...
`pr-review/gate`, using the check names reported by GitHub. App installations,
